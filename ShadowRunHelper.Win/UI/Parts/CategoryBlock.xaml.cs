...
namespace ShadowRunHelper.UI
{
    public sealed partial class CategoryBlock : UserControl
    {
        ...
        private async void UI_TxT_CSV_Cat_Import_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                strRead = (await SharedIO.CurrentIO.LoadFileContent(new FileInfoClass(Place.Extern,"","") { Token = "import"}, Constants.LST_FILETYPES_CSV, UserDecision.AskUser)).strFileContent;
            }
        ...
        private void UI_TxT_CSV_Cat_Export_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SharedIO.CurrentIO.SaveFileContent(output, new FileInfoClass(Place.Extern, TypeHelper.ThingDefToString(Controller.eDataTyp, true) + Constants.DATEIENDUNG_CSV,"") { Token = "CSV_TEMP" });
            }
            catch (IsOKException ex)
            {
                return;
